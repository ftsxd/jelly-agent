...
// The console's declaration takes effect without discarding the rest of the
// entry it lands on.
//
// Two ways to get this wrong, and the first version had one of each. Loading
// the console's file after the others meant a declaration made by clicking
// simply lost to whatever a hand-written file said — silently, while the page
// reported 已声明. Loading it first meant it won by replacing: registration is
// first-wins per key, so its four fields stood in for the whole entry and the
// description, the timeout, the result ceiling and the aliases went with it.
//
// It is neither. It is a patch.
func TestTheConsoleFilePatchesRatherThanReplaces(t *testing.T) {
	dir := t.TempDir()
	// Sorts before console.yaml, which is how the hand-written file used to
	// win on filename alone.
	hand := `tools:
  - name: query_range
    server: n9e
...
`
	...
	if err := os.WriteFile(filepath.Join(dir, ConsoleFile), []byte(console), 0o644); err != nil {
		t.Fatal(err)
	}

	metas, err := NewFileSource(dir).Load(t.Context())
	if err != nil {
		t.Fatal(err)
	}
	if len(metas) != 1 {
		t.Fatalf("metas = %d, want the console entry folded into the file entry: %+v", len(metas), metas)
	}
	got := metas[0]
	if got.Produces != ops.KindMetricSeries {
		t.Errorf("produces = %q；控制台上刚点的那个没有生效", got.Produces)
	}
	// Everything the console does not know about has to survive it.
	if got.Description == "" {
		t.Error("描述被控制台的声明覆盖掉了——模型读的就是它")
	}
	if got.Timeout == 0 || got.MaxResultBytes == 0 || len(got.Aliases) != 1 {
		t.Errorf("整条元数据被替换了: timeout=%v max=%d aliases=%v",
			got.Timeout, got.MaxResultBytes, got.Aliases)
	}
	// A field the console left alone keeps the file's answer.
	if got.SideEffect != ops.SideEffectReadOnly {
		t.Errorf("side_effect = %q；控制台没有说过它", got.SideEffect)
	}

	reg, conflicts := Build(metas)
	if len(conflicts) != 0 {
		t.Errorf("冲突 = %+v；两份声明本该合成一条，而不是互相顶撞", conflicts)
	}
	m, ok := reg.Lookup("query_range")
	if !ok || m.Produces != ops.KindMetricSeries {
		t.Errorf("registry = %+v/%v", m, ok)
	}
}

// A declaration for a tool no file mentions stands on its own — the common
// case, since an MCP tool has no file entry unless somebody wrote one.
func TestAConsoleDeclarationForAnUnknownToolStandsAlone(t *testing.T) {
	dir := t.TempDir()
	...
	if len(metas) != 1 || metas[0].Produces != ops.KindWorkloadStatus {
