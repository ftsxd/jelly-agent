...
- **配置热重载**：Web 端增删改 Provider/MCP/消息绑定保存即生效；直接编辑磁盘上的 `config.yaml` 也会被监听到并自动热重载，对话不中断、无需重启。「不中断」是被保证的而不是碰运气：换掉的旧引擎不立刻关闭，它持有数据库连接、检索索引和 MCP 子进程，正跑在它上面的请求（包括还没结束的 SSE 对话轮）用完之后才关。
