...
本地构建可使用 `make check`、`make build`；Docker 镜像先通过 `make docker` 构建一次，之后使用 `docker compose up -d` 启动，无需每次重新构建。首次容器启动会生成管理员初始密码，请从容器日志获取并立即修改。Compose 默认将控制台发布在宿主机的 `6185` 端口；公网部署应置于 HTTPS 反向代理之后。
