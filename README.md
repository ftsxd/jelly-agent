...
每轮对话都会把完整历史发给模型，几次 `fetch_url`（单次最多 8000 字符）就能顶满上下文窗口。压缩层在请求发出前把历史裁进预算，**确定性执行、不调用模型做摘要**，因此不增加延迟、费用与新的失败点。

按「代价从小到大」逐级升级：

1. 精简较早的**工具结果**（体量最大、最不需要逐字重读的部分），保留首尾并标注 `truncated`；
2. 仍超预算则**整段丢弃**较早的对话，并插入一条说明「已省略 N 条」的提示，让模型知道历史被截短过而不是从未发生；
3. 最后手段——连受保护的尾部也精简工具结果，但**永不丢弃尾部**，当前问题必定送达模型。

配置在 `config.yaml` 的 `history` 段（`max_tokens` 填 0 关闭）。发生压缩时会打印一行 `[jelly] 上下文压缩 (root): 31200→18400 token，省略 12 条、截断 3 个工具结果`。
...
**Agent 变量**：在 Web **Agents** 页给单个 Agent 配变量（`KEY=VALUE`，存 `config.yaml` 的 `agent_vars`），该 Agent 调 `run_script` 时注入脚本环境，叠在技能自己的变量之上，**同名时 Agent 的赢**——同一个技能被几个 Agent 共用、每个要带自己那份凭据时用它。同样支持 `${ENV}` 引用（值不落盘，从服务端进程环境取），Web 只回显键名。变量走独立接口保存，所以列表页上点启停/设默认不会把它们清掉。
...
- 密钥**只进子进程环境**，不进 system prompt、不进 `use_skill` 输出、不进会话记录。
- **输出侧也拦**：脚本输出里凡是出现注入值的地方，沙箱在返回前替换成 `${变量名}`，所以脚本 `echo` 了、`set -x` 了、报错信息里带了，都不会进对话和会话记录（4 字符以内的值不替换——太短，会误伤正常输出）。这挡的是**无意**泄露；一个存心外传的脚本总能 base64 一下，那种情况靠的是别导入没读过的技能包。
- ⚠️ 上面这些的前提是沙箱真的生效。`sandbox.backend: native` 或 `mode: full` 只剩环境隔离，文件那道闸没了——脚本能直接读 `~/.jelly-agent/config.yaml`，那里面是**全部**密钥。线上别退到这两档。
