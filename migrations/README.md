...
**源库有目标库没有的列时，它会拒绝并报出列名**，而不是照搬其余的。迁移是那种
...
第一版数字是 872ms / 1531ms / 1207ms，比现在差两个数量级。三个问题都和 PG
无关，只是被 PG 放大 —— 在 SQLite 上同样存在，便宜到看不见。

**① 每个请求重建一次 ADK 会话服务。** 六个 handler 都调
`NewSessionService()`，而它每次都开连接 + 跑 `AutoMigrate`：

```
sqlite    857 µs 一次
postgres  733 ms 一次     ← 时间线总耗时的 85%
```

改成 engine 里 `sync.Once` 建一次。

**② 会话列表给每一行做一次完整会话加载。** `sessionPreview` 为了显示一行
预览把该会话全部事件读出来，一页 20 行就是 20 次。首条用户消息在会话创建后
不会再变，所以能缓存（`internal/server/preview.go`）。

**③ 任务列表给每个会话做一次完整加载 + 一次链接查询。**

链接改成一条 `task.OfSessions(db, ids)`。加载这一半用投影缓存
（`internal/server/framecache.go`），键是 `(session, lastUpdate, eventCount)`：

- **只缓存昂贵的那一半**——事件加载 + `projectAll`。折叠成任务需要工具元数据
  和任务归属，两者都会在会话不变时改变；缓存里放任务就得知道"元数据被编辑了"，
  而一个需要被告知第二种变化的缓存，其失效逻辑没人能推清楚。**帧只依赖事件。**
- **键里带事件数**，因为 `ListPage` 的更新时间是整秒的——同一秒内的两条事件
  会共用一个时间戳。ADK 只追加不改写，所以变过的日志要么秒数更大、要么事件更多。
- 详情页传零版本，即"不缓存"：它手上没有版本，而一个用无法比较的版本缓存起来
  的投影会静默变旧。
...
| | 会话时间线 | 任务列表 | 会话列表 |
|---|---|---|---|
| SQLite | 378 µs | 2.2 ms | 2.1 ms |
| PostgreSQL | **3.9 ms** | **8.7 ms** | **6.6 ms** |
| 倍数 | 10× | 4× | 3× |

对比最初：任务列表 1531ms → 8.7ms，会话时间线 872ms → 3.9ms。

`taskScanMax = 600` 那条最坏路径（过滤条件匹配不到时扫到上限）以前是
600 × 20ms = 12 秒，现在第二次之后是零次会话加载 —— 有测试守着
（`TestAFilterThatMatchesNothingDoesNotReloadEverySession`）。
