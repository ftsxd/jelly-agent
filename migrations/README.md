...
**但"已存在"的行会核对内容，不是数一数就算了。** `ON CONFLICT DO NOTHING`
保留的是目标库里那一行，所以行数照样对得上，而它说的可能和源库不是一回事——
这正是行数核对天生看不见的那种错。插入时用 `RETURNING` 主键，没被返回的就是
目标库已有的行，按主键读回来逐列比；主键相同而内容不同就报出列名和两边的值，
并且中止。两种情况会撞上它：搬错库了，或者服务已经在目标库上跑过、把它写变了
（后者本来就不该再搬第二次）。

比较要跨方言，所以时间戳（SQLite 是 RFC3339 文本，PG 是 `timestamptz`）、
布尔（0/1 对 true/false）、JSON（`jsonb` 会重排键、加空格）都先归一化再比。
不归一化的话每一行都会被判成冲突——那比不比更糟，等于禁掉了重跑。
...
`tool_decl_log` 一表两用：既是审计（谁在什么时候改了哪个字段，带整份快照），
也是失效通知。每个进程按 3 秒轮询它的 `MAX(id)`，一动就重建自己的工具注册表 ——
所以一个控制台里改的声明，另一个进程不用重启就能看到。这正是这一层从
`console.yaml` 搬进数据库的理由：文件的 mtime 只有本进程看得见。

轮询的那个版本号只在**注册表真的装上之后**才前进，而且由装它的那段代码自己
持有。读一次、装一次、再记下版本，全在同一个协程里顺着写下来 —— 中间没有任何
地方可以把这次变更算作处理过而实际上没装上。加载失败、重建失败、库还没起来，
都只是耽误一轮：基线没动，下一轮照样重来。
...
- **schema 已经有两份定义了**（各 store 的 DDL、这个迁移文件）。一个自带列
  ...
  和类型**。
- **核对得和搬运在一起。** 没法核对的迁移就是没法信任的迁移。而且内容核对
  只能在搬运时做：哪些行是"目标库已有"的，只有插入那一刻知道，事后再看，
  行数一样的两张表已经说不出谁是谁了。
...
5. 再跑一次 migrate      → 复制 0 行，已存在 115 行，逐列核对无冲突
