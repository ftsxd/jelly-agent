...
第一版数字是 872ms / 1531ms / 1207ms，比现在差一个数量级。查下来是两个和 PG
无关、只是被 PG 放大的问题：
...
改成 engine 里 `sync.Once` 建一次。时间线 872ms → 19.7ms。

**② 会话列表给每一行做一次完整会话加载。** `sessionPreview` 为了显示一行
预览，把该会话的全部事件都读出来，一页 20 行就是 20 次全量加载。而首条用户
消息在会话创建后**不会再变**，所以能缓存（`internal/server/preview.go`，
LRU + 删除时失效）。会话列表 480ms → 9.3ms。

两个在 SQLite 上都存在，只是便宜到看不见。

### 任务列表为什么还是 642ms

不是 bug，是设计特征：任务是从 events 投影出来的，`tasksOf` 对扫到的每个
会话做一次完整 `svc.Get`。

```
postgres  单个会话 Get（30 条事件）  20.3 ms
24 个会话 × 20.3ms ≈ 487ms          ← 642ms 的主体
```

能缓存，但键要带 `LastUpdate`（任务会随会话变），而投影还依赖工具元数据
（`infoOf`）——元数据一改缓存就得失效。这个失效逻辑没验透之前不该建：一个
说不清何时失效的缓存，比慢一点更糟。

顺带一提，`taskScanMax = 600`：一个匹配不到东西的过滤条件会扫 600 个会话，
在 PG 上就是 600 × 20ms = 12 秒。这条路径现在没有测试，也是同一个 N+1。
